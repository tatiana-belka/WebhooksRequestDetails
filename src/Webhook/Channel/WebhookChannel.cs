...
using Webhook.Channel.HubInvoker;

namespace Webhook.Channel
{
    public class WebhookChannel : IDisposable
    {
        ...
        public WebhookChannel(ILoggerFactory loggerFactory)
        {
            _logger = loggerFactory.CreateLogger<WebhookChannel>();
        }
        ...
        public async Task ManageAsync(WebSocket socket, IHub hub)
        {
            ...
            try
            {
                var establishConnectionAsync = typeof(WebhookChannel).GetMethod(nameof(EstablishConnectionAsync), BindingFlags.Instance | BindingFlags.NonPublic)!;
