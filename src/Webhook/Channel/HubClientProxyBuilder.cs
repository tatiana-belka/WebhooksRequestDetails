...
namespace Webhook.Channel
{
    internal class HubClientProxyBuilder
    {
            ...
            where TClient : IHubClient
        {
            ...
            var proxyTypeName = "Webhook.Generated.HubClientProxy." + targetInterfaceType.FullName + "." + (_sequenceId++);
            var typeBuilder = _moduleBuilder.DefineType(proxyTypeName, TypeAttributes.Class, hubClientBaseType);

           
            var baseCtor = hubClientBaseType.GetConstructor(new[] { typeof(WebhookChannel) })!;
